...
def multiple(bb: Bitboard) -> bool:
    return bb & (bb - 1)

def scan_forward(bb: Bitboard):
    while bb:
        r = bb & -bb
        yield r.bit_length() - 1
        bb ^= r

def shift(bb: Bitboard, direction: int) -> Bitboard:
    bb = {
        1: bb & ~H_FILE,
        7: bb & ~A_FILE
    }.get(direction & 7, bb)
    ...
    if direction > 0:
        return uint64(bb << direction)
    else:
        return bb >> -direction

def all_pawn_attacks(pawns: Bitboard, side: int) -> Bitboard:
    up = (NORTH, SOUTH)[side]
    return shift(pawns, up + EAST) | shift(pawns, up + WEST)
